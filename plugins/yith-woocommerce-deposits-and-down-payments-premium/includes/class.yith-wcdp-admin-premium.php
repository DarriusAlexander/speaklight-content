	...
	class YITH_WCDP_Admin_Premium extends YITH_WCDP_Admin {
		...
		public function render_metabox_output( $post, $param ) {
			...
			if ( $order ) {
				...
				foreach( $suborders as $suborder_id ){
					...
					if( ! yit_get_prop( $suborder,'_has_expired' ) && ! in_array( $suborder->get_status(), array( 'completed', 'processing', 'cancelled' ) ) ){
						$send_available = true;
					}
				}
			}
			...
			switch ( $param['args']['metabox'] ) {
				case 'suborders':
					if( $deposit_expire == 'yes' && $notification_days && $send_available ){
		...
		public function bulk_action() {
			...
			foreach ( $post_ids as $post_id ) {
				do_action( 'yith_wcdp_deposits_expiring', $post_id );
				$changed ++;
			}

			$sendback = add_query_arg( array( 'post_type' => 'shop_order', 'changed' => $changed, 'ids' => join( ',', $post_ids ) ), '' );
		...
		public function resend_notification_email() {
			if( isset( $_GET['order_id'] ) && isset( $_GET['resend_notification_email_nonce'] ) && wp_verify_nonce( $_GET['resend_notification_email_nonce'], 'resend_notification_email' ) ){
				...
				do_action( 'yith_wcdp_deposits_expiring', $order_id );
		...
		public function print_item_to_refund_notice( $item_id, $item ) {
			...
			if( isset( $item['deposit_needs_manual_refund'] ) && $item['deposit_needs_manual_refund'] && $schedule == 'yes' ){
				$create_refund_for_item_url = esc_url( add_query_arg( array( 'action' => 'yith_wcdp_refund_item', 'order_id' => $order_id, 'item_id' => $item_id ), admin_url( 'admin.php' ) ) );
				$hide_notice_for_item_url = esc_url( add_query_arg( array( 'action' => 'yith_wcdp_delete_refund_notice', 'order_id' => $order_id, 'item_id' => $item_id ), admin_url( 'admin.php' ) ) );

				?>
				<div class="yith-wcdp-to-refund-notice error-notice">
					<p>
						<small>
							<?php echo sprintf( __( 'This item should be manually refunded by admin, since the %d days available to complete payment have passed and deposit has expired', 'yith-woocommerce-deposits-and-down-payments' ), $expiration ) ?>
							<a href="<?php echo $create_refund_for_item_url ?>"><?php _e( 'Create refund', 'yith-woocommerce-deposits-and-down-payments' ) ?></a> | <a href="<?php echo $hide_notice_for_item_url ?>"><?php _e( 'Hide this notice', 'yith-woocommerce-deposits-and-down-payments' )?></a>
						</small>
					</p>
				</div>
				<?php
		...
		public function add_to_refund_deposit_view( $views ) {
			...
			if( $order_to_refund_count ){
				$filter_url = esc_url( add_query_arg( array( 'post_type' => 'shop_order', 'deposit_to_refund' => true ), admin_url( 'edit.php' ) ) );
		...
		public function print_bulk_editing_fields( $column_name, $post_type ) {
			...
			if( $post_type != 'product' || $column_name != 'product_type'){
				return;
			}
		 ...
		 * @since 1.0.2
		 */
		public function save_bulk_editing_fields( $post_id ) {
            $is_yith_bulk_editing = isset( $_REQUEST[ 'action' ] ) && 'yith_wcbep_bulk_edit_products' === $_REQUEST[ 'action' ];
            if ( $is_yith_bulk_editing )
                return;
		...
		public function save_product_deposit_tabs( $post_id ) {
			...
			$create_balance_orders = ( isset( $_POST['_create_balance_orders'] ) && in_array( $_POST['_create_balance_orders'], array( 'yes', 'no' ) ) ) ? $_POST['_create_balance_orders'] : 'default';
			$deposit_default = ( isset( $_POST['_deposit_default'] ) && in_array( $_POST['_deposit_default'], array( 'yes', 'no' ) ) ) ? $_POST['_deposit_default'] : 'default';
		...
		public function save_variation_deposits_settings( $variation_id, $loop ) {
			...
			$create_balance_orders = ( isset( $_POST['_create_balance_orders'][ $loop ] ) && in_array( $_POST['_create_balance_orders'][ $loop ], array( 'yes', 'no' ) ) ) ? $_POST['_create_balance_orders'][ $loop ] : 'default';
			$deposit_default = ( isset( $_POST['_deposit_default'][ $loop ] ) && in_array( $_POST['_deposit_default'][ $loop ], array( 'yes', 'no' ) ) ) ? $_POST['_deposit_default'][ $loop ] : 'default';
		...
		public function add_premium_general_settings( $settings ) {
			...
			$deposit_general_option = array(
				'general-create-balance-orders' => array(
					'title' => __( 'Let users pay balances online', 'yith-woocommerce-deposits-and-down-payments' ),
					'type' => 'checkbox',
					'desc' => __( 'Check this option to create balance orders with "Pending Payment" status, so that users can complete purchases online; otherwise "On Hold" status will be applied (this behaviour can be overridden in product page)', 'yith-woocommerce-deposits-and-down-payments' ),
