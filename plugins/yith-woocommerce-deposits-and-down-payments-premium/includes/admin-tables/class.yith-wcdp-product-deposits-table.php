...
if ( ! class_exists( 'YITH_WCDP_Product_Deposits_Table' ) ) {
	...
	class YITH_WCDP_Product_Deposits_Table extends WP_List_Table {
		...
		public function column_product( $item ) {
			...
			$column = sprintf( '%s<a href="%s">%s</a>%s', $product->get_image( array( 32, 32 ) ), get_edit_post_link( $item['ID'] ), $item['name'], $attributes );
		...
		public function prepare_items() {
			...
			if( ! empty( $registered_deposits ) ){
				...
				$column_order = isset( $_REQUEST['orderby'] ) && in_array( $_REQUEST['orderby'], array( 'product_names', 'product_prices', 'product_values', 'product_types' ) ) ? $_REQUEST['orderby'] : 'product_names';
