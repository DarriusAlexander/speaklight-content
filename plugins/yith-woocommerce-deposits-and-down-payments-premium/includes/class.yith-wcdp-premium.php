...
if ( ! class_exists( 'YITH_WCDP_Premium' ) ) {
	...
	class YITH_WCDP_Premium extends YITH_WCDP {
		...
		public function is_deposit_enabled_on_product( $product_id = false, $variation_id = false ) {
			...
			return apply_filters( 'yith_wcdp_is_deposit_enabled_on_product', $plugin_enabled == 'yes' && $deposit_enabled == 'yes', $product_id, $variation_id );
		}
		...
		public function get_deposit( $product_id, $price = false, $context = 'edit', $customer_id = false, $variation_id = false ) {
			...
			elseif( $deposit_type == 'amount' ){
				$deposit_value = 'view' == $context ? yith_wcdp_get_price_to_display( $product, array( 'qty' => 1, 'price' => $deposit_amount ) ) : $deposit_amount;
			}
		...
		public function enable_resend_notify_email( $resend_emails ) {
			...
			foreach( $suborders as $suborder_id ){
				...
				if( ! yit_get_prop( $suborder, 'has_expired' ) && ! in_array( $suborder->get_status(), array( 'completed', 'processing', 'cancelled' ) ) ){
			...
			if( $resend_available ){
				if( version_compare( WC()->version, '3.2.0', '>=' ) ) {
					$resend_emails['expiring_deposit'] = __( 'Email expiring deposit notification to customer', 'yith-woocommerce-deposits-and-down-payments' );
				}
				else {
		...
		public function update_cart_item( $cart_item ) {
			...
			if( $this->is_deposit_enabled_on_product( $product, $variation_id ) && ! yit_get_prop( $product, 'yith_wcdp_deposit', true ) && ! apply_filters( 'yith_wcdp_skip_cart_item_processing', false, $cart_item ) ){
		...
		public function update_cart_item_data( $cart_item_data, $product_id, $variation_id ) {
			...
			if( $this->is_deposit_enabled_on_product( $product_id, $variation_id ) && ! apply_filters( 'yith_wcdp_skip_cart_item_processing', false, $cart_item_data ) ){
				$deposit_forced = $this->is_deposit_mandatory( $product_id, $variation_id );
