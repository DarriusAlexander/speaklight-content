    ...
    class YITH_WC_Subscription_Status extends WC_Email {
        ...
        public function trigger( $subscription ) {
	        ...
	        if ( version_compare( WC()->version, '3.2.0', '<' ) ) {
		        ...
		        $this->replace['subscription-status'] = isset( $status[ $subscription->status ]) ? $status[ $subscription->status ] : $status[ $subscription->status ];
	        }else{
		        $this->placeholders['{subscription_id}'] = $subscription->id;
		        $this->placeholders['{status}'] = isset( $status[ $subscription->status ]) ? $status[ $subscription->status ] : $status[ $subscription->status ];
	        }
