...
if ( ! class_exists( 'YWSBS_Subscription' ) ) {
	...
	class YWSBS_Subscription {
		...
		public function update( $order_id ) {
			...
			yit_save_prop( $parent_order , array(
				'failed_attemps'       => 0,
		...
		public function update_prices( $posted ) {
			...
			if( $changes ) {
				...
				foreach ( $changes as $key =>$change ) {
					...
					$message .= sprintf( __( '%s from %s to %s', 'yith-woocommerce-subscription' ), str_replace( '_', ' ', $key ), $old_values[ $key ] . "{$currency}", $new_values[ $key ] . "{$currency}<br>" );
				}
		...
		private function calculate_taxes() {
			...
			$this->set( 'line_tax_data', array( 'subtotal' => $subtotal_taxes, 'total' => $taxes ) );
		}
		...
		public function get_customer_order_note(){
			return !empty( $this->customer_note ) ? $this->customer_note : yit_get_prop( $this->get_order(), 'customer_note' );
		}
		...
		public function get_billing_email() {
			$billing_email = ! empty( $this->billing_email ) ? $this->billing_email : yit_get_prop( $this->get_order(), 'billing_email');
		...
		public function get_billing_phone() {
			$billing_phone = ! empty( $this->billing_phone ) ? $this->billing_phone : yit_get_prop( $this->get_order(), 'billing_phone' );
		...
		public function register_failed_attempt( $attempts = false, $latest_attempt = false, $next_attempt_date = '' ) {
			...
			if ( 'yes' == get_option( 'ywsbs_suspend_for_failed_recurring_payment' ) ) {
				if ( $this->get( 'status' ) != 'suspended' ) {
					...
					YITH_WC_Subscription()->log( sprintf( __( 'Subscription suspended. Order %s. Subscription %s', 'yith-paypal-express-checkout-for-woocommerce' ), $order_id, $this->id ) );
				}
			}
