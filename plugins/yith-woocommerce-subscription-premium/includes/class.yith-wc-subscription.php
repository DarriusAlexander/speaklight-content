...
if ( ! class_exists( 'YITH_WC_Subscription' ) ) {
	...
	class YITH_WC_Subscription {
		...
		public function __construct() {
			...
			add_filter( 'woocommerce_show_variation_price', array( $this, 'show_variation_subscription_price'), 30, 3 );
		...
		public function change_general_price_html( $product, $quantity = 1 ) {
			...
			if ( ! $product->is_type('variable') &&
				...
				$and = false;
				$price_html .= ( $signup_fee || $trial_period ) ? __( '<span class="ywsbs-price-detail"> + a ', 'yith-woocommerce-subscription' ) : '';

				if ( $signup_fee && get_option( 'ywsbs_show_fee' ) == 'yes' ) {
					$price_html .= $and ? __( ' and ', 'yith-woocommerce-subscription' ) : '';
					$price_html .= apply_filters( 'ywsbs_signup_fee_label', __( ' sign-up fee of ', 'yith-woocommerce-subscription' ) ) . wc_price( $signup_fee );
					$and = true;
				}
				...
				if ( $trial_period && get_option( 'ywsbs_show_trial_period' ) == 'yes' ) {
					$price_html .= $and ? __( ' and ', 'yith-woocommerce-subscription' ) : '';
					$price_html .= __( ' free trial of ', 'yith-woocommerce-subscription' ) . $t;
				}
		...
		public function change_price_html( $price, $product, $order = null ) {
			...
			if ( ! $product->is_type('variable') &&
				...
				if ( $signup_fee && get_option( 'ywsbs_show_fee' ) == 'yes' ) {
					...
					$price .= apply_filters( 'ywsbs_signup_fee_label', __( ' sign-up fee of ', 'yith-woocommerce-subscription' ) ) . wc_price( $signup_fee, array( 'currency' => $currency ) );
					$and = true;
				}
				...
				if ( $trial_period && get_option( 'ywsbs_show_trial_period' ) == 'yes' ) {
					$price .= $and ? __( ' and ', 'yith-woocommerce-subscription' ) : '';
					$price .= __( ' free trial of ', 'yith-woocommerce-subscription' ) .  $t;
				}
		...
		public function cart_item_validate( $valid, $product_id, $quantity, $variation_id = 0 ) {
			...
			if ( $this->is_subscription( $product ) && $item_key = $this->cart_has_subscriptions() ) {
		...
		public function my_account_edit_address(  ) {
			...
			if ( isset( $_GET['subscription'] ) ) {
				...
				woocommerce_form_field( 'change_subscriptions_addresses', array(
						'type'  => 'checkbox',
		...
		public function downgrade_process( $from_id, $to_id, $subscription ) {
			...
			if ( $left_time <= 0 && $days > 1 ) {
				add_user_meta( $subscription->user_id, 'ywsbs_upgrade_' . $to_id, array(
					'subscription_id' => $subscription->id,
					'pay_gap'         => 0
				) );
			} elseif ( $left_time > 0 ) {
				add_user_meta( $subscription->user_id, 'ywsbs_downgrade_' . $to_id, $subscription->id );
				add_user_meta( $subscription->user_id, 'ywsbs_trial_' . $to_id, array(
					'subscription_id' => $subscription->id,
					'trial_days'      => $days
				) );
			}
		...
		public function load_wc_mailer() {
			...
			//Customers
			add_action( 'ywsbs_customer_subscription_cancelled_mail', array(
				'WC_Emails',
				'send_transactional_email'
			), 10 );
			add_action( 'ywsbs_customer_subscription_expired_mail', array(
				'WC_Emails',
				'send_transactional_email'
			), 10 );
			add_action( 'ywsbs_customer_subscription_before_expired_mail', array(
				'WC_Emails',
				'send_transactional_email'
			), 10 );
			add_action( 'ywsbs_customer_subscription_suspended_mail', array(
				'WC_Emails',
				'send_transactional_email'
			), 10 );
			add_action( 'ywsbs_customer_subscription_resumed_mail', array(
				'WC_Emails',
				'send_transactional_email'
			), 10 );
			add_action( 'ywsbs_customer_subscription_paused_mail', array(
				'WC_Emails',
				'send_transactional_email'
			), 10 );
			add_action( 'ywsbs_customer_subscription_request_payment_mail', array(
				'WC_Emails',
				'send_transactional_email'
			), 10 );
			add_action( 'ywsbs_customer_subscription_renew_reminder_mail', array(
				'WC_Emails',
				'send_transactional_email'
			), 10 );
			add_action( 'ywsbs_customer_subscription_payment_done_mail', array(
				'WC_Emails',
				'send_transactional_email'
			), 10 );
			add_action( 'ywsbs_customer_subscription_payment_failed_mail', array(
				'WC_Emails',
				'send_transactional_email'
			), 10 );
