...
class YITH_YWSBS_Subscriptions_List_Table extends WP_List_Table {
	...
	public function extra_tablenav( $which ) {
		if ( 'top' == $which ) {
			...
			foreach ( $status as $key => $value ) {
				$q = $wpdb->get_var( $wpdb->prepare( "SELECT count(*) as counter FROM $wpdb->posts as ywsbs_p INNER JOIN " . $wpdb->prefix . "postmeta as ywsbs_pm ON ( ywsbs_p.ID = ywsbs_pm.post_id )
                  WHERE ywsbs_pm.meta_key = 'status' AND ywsbs_pm.meta_value = '%s'", $key ) );
			...
			foreach ( $gateways as $key => $gateway) {
				$q = $wpdb->get_var( $wpdb->prepare( "SELECT count(*) as counter FROM $wpdb->posts as ywsbs_p INNER JOIN " . $wpdb->prefix . "postmeta as ywsbs_pm ON ( ywsbs_p.ID = ywsbs_pm.post_id )
                  WHERE ywsbs_pm.meta_key = 'payment_method' AND ywsbs_pm.meta_value = '%s'", $key ) );
	...
	protected function get_views() {
		...
		foreach ( $status as $key => $value ) {
			$q = $wpdb->get_var( $wpdb->prepare( "SELECT count(*) as counter FROM $wpdb->posts as ywsbs_p INNER JOIN " . $wpdb->prefix . "postmeta as ywsbs_pm ON ( ywsbs_p.ID = ywsbs_pm.post_id )
                  WHERE  ywsbs_p.post_status= 'publish' AND ywsbs_pm.meta_key = 'status' AND ywsbs_pm.meta_value = '%s'", $key ) );
