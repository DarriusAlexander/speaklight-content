...
if ( ! function_exists( 'ywsbs_get_order_fields_to_edit' ) ) {
	...
	function ywsbs_get_order_fields_to_edit( $type ) {
		...
		if ( 'billing' == $type ) {
			...
			$fields = apply_filters( 'ywsbs_admin_billing_fields', array(
				'first_name' => array(
			...
			$fields = apply_filters( 'ywsbs_admin_shipping_fields', array(
				'first_name' => array(
					'label' => __( 'First name', 'woocommerce' ),
					'show'  => false,
				),
	...
	function ywsbs_check_renew_order_before_pay( $order ) {
		...
		if ( $is_valid ) {
			...
			if ( ! $order->has_status( $status ) ) {
				YITH_WC_Subscription()->log( sprintf( __( 'New payment request denied because the order #%d is on %s status', 'yith-woocommerce-subscription' ), $order_id, $order->get_status() ) );
				$is_valid = false;
			}
		}
...
if ( ! function_exists( 'ywsbs_register_failed_payment' ) ) {
	...
	function ywsbs_register_failed_payment( $order, $error_message ) {
		...
		$order->add_order_note( sprintf( __( '%s Failed payment: %s', 'yith-woocommerce-subscription' ), $payment_method_title, $error_message ) );
		...
		if ( $is_a_renew == 'yes' ) {
			...
			if ( $failed_attempts + 1 < $max_attempts ) {
				...
				YITH_WC_Subscription()->log( sprintf( __( '%s - total failed attempts (%s) for order #%s', 'yith-woocommerce-subscription' ), $payment_method_title, ( $failed_attempts + 1 ), $order_id ) );
				...
				foreach ( $subscriptions as $subscription_id ) {
					...
					YITH_WC_Subscription()->log( sprintf( __( 'Subscription cancelled for max failed attempts. Subscription #%s. Order #%s', 'yith-woocommerce-subscription' ), $subscription_id, $order_id ) );
				}
			}
		}
	}
