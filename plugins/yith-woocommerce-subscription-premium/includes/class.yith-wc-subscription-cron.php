...
if ( !class_exists( 'YWSBS_Subscription_Cron' ) ) {
    ...
    class YWSBS_Subscription_Cron {
	    ...
	    public function set_cron(){
		    ...
		    if ( ywsbs_enable_scheduling() && !wp_next_scheduled( 'ywsbs_pay_renew_subscription_orders' ) ) {
	    ...
	    public function ywsbs_pay_renew_subscription_orders(  ) {
		    ...
		    $query = $wpdb->prepare( "SELECT ywsbs_p.ID FROM {$wpdb->prefix}posts as ywsbs_p
                 ...
                 AND ( ywsbs_pm2.meta_key='failed_attemps' AND ywsbs_pm2.meta_value = 0 ) 
                 GROUP BY ywsbs_p.ID ORDER BY ywsbs_p.ID DESC
                ", 'shop_order', $status );
		    ...
		    $query = $wpdb->prepare( "SELECT ywsbs_p.ID FROM {$wpdb->prefix}posts as ywsbs_p
                 ...
                 GROUP BY ywsbs_p.ID ORDER BY ywsbs_p.ID DESC", 'shop_order', $status, $current_time  );
		    ...
		    if ( $renew_orders ) {
			    foreach ( $renew_orders as $renew_order ) {
				    ...
				    if ( ywsbs_check_renew_order_before_pay( $current_order ) ) {
