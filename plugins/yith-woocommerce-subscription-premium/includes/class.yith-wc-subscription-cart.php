...
if ( ! class_exists( 'YWSBS_Subscription_Cart' ) ) {
	...
	class YWSBS_Subscription_Cart {
		...
		public function remove_change_prices_filter(  ) {
			remove_filter( 'woocommerce_product_get_price', array( $this, 'change_prices_for_calculation'), 100, 2 );
			remove_filter( 'woocommerce_product_variation_get_price',array( $this, 'change_prices_for_calculation'), 100, 2 );
		}
		...
		public function change_prices_for_calculation( $price, $product ) {
			...
			if( ! YITH_WC_Subscription()->is_subscription( $product->get_id() ) || $is_raq   ){
				return $price;
			}
					...
					if ( apply_filters( 'ywsbs_price_check', true, $product, $price ) ) {
						...
						if ( isset( WC()->cart->cart_contents[ $cart_item_key ] ) ) {
							if ( version_compare( WC()->version, '2.7.0', '>=' ) ) {
								...
								//var_dump( WC()->cart->cart_contents[ $cart_item_key ]['data'] );
			...
			if ( YITH_WC_Subscription()->is_subscription( $product_id ) ) {
				...
				if( is_checkout() && 'yes' == get_option('ywsbs_subscription_total_amount', 'no')){
					...
					if( $max_lenght && $max_lenght > 1 ){
						$price_html .= apply_filters('ywsbs_checkout_subscription_total_amount', '<br><small>'.sprintf( __( '(Subscription total for %s %s: %s)', 'yith-woocommerce-subscription' ), $max_lenght, $price_time, wc_price( $price * $cart_item['quantity'] * $max_lenght, $price_time  ) ) .'</small>' ) ;
					}
				}
			}
