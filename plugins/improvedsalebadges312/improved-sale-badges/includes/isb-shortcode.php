<?php
	...
	class WC_Isb_Shortcodes {
		...
		public static function get_badge( $atts, $content = null ) {
			...
			return '<div' . ( $atts['shortcode_id'] !== '' ? ' id="' . esc_attr( $atts['shortcode_id'] ) .'"' : '' ) . ' class="isb-sc' . $class . '">' . ob_get_clean() . '</div>';

		}

	}

	add_action( 'init', array( 'WC_Isb_Shortcodes', 'init' ) );
