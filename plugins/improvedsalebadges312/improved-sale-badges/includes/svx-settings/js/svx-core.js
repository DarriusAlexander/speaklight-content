(function($){
	...
	function make_menu() {
		...
		if ( !u(svx.license) ) {
			$('#svx-settings-menu').before( '<p class="license">'+svx.name+' plugin license is not validated! <a href="'+svx.link+'" class="svx-button">Get a License</a> <span class="svx-button-primary svx-license">Validate plugin license here!</span></p>' );
		}
		else {
			$('#svx-main-header .svx-main-buttons').append( '<a href="https://mihajlovicnenad.com/support/" class="svx-button-primary" target="_blank">Official Support</a>');
		}
	}
	...
	$(document).on( 'click', function(e) {
		...
		if (e.target && e.target.matches('.svx-license')) {
			$('#svx-settings-menu li[data-id="license"]').trigger('click');
		}
		...
		/*if (e.target && e.target.id == 'svx-edit-terms') {
			show_term_customizer(e);
		}*/
	...
	$(document).on( 'keyup', '#svx-settings-wrapper, #svx-customizer', function(e) {
		...
		/*if ( e.target && e.target.name.toString() == 'name' ) {
			set_term_name(e);
		}*/
	...
	function set_term_name(e) {
		var $e = $(e.target);
		$e.closest('.svx-terms-list-item').find('.svx-term-item-title:first').html($e.val());
	}
	...
	function make_control_options(e,f) {
		...
		$.each( dO, function(i,g) {
			...
			var tmplData = {
				val: i,
				name: g,
				sel: f.val!==false?f.val:f.default
			};
...
/*changed to [] check that */
		...
		if ( typeof p[0] !== 'undefined' ) {
			...
			if ( typeof p[1] !== 'undefined' ) {
				...
				if ( u(svx.settings[g].val[p[0]].options[p[1]]) === false ) {
					...
					var chk = e.target.dataset.type !== false ? svx.settings[g].settings.options.settings[e.target.dataset.type] : svx.settings[g].settings.options.settings;
					$.each( chk, function(n,m) {
	...
	function get_field_original(e,f) {
		var a = get_field_path(e,f.id).split('[')
	...
	function expand_list_item(e) {
		...
		if ( typeof s.ajax_options !== 'undefined' ) {
	...
	function add_list_items(e,f) {
		...
		$.each( f.val, function(b,c) {
			...
			if ( typeof c !== 'undefined' ) {
				...
				var tmplData = {
					title: u(c.name) !== false && c.name !== '' ? c.name : 'Not set',
					type: u(c.type),
					customizer: u(f.supports)!==false&&$.inArray('customizer',f.supports)!==-1?true:false,
					options: ''
				};
		...
		switch ( p.length ) {
			case 4 :
				if ( v===null ){
					...
					//delete flx[p[1]][p[2]][p[3]];
					blx = __return_deleted_array(flx) /* flx[p[1]][p[2]] */
				}
				else {
					blx = flx;
					blx[p[1]][p[2]][p[3]] = v;
			...
			case 3 :
				if ( v===null ){
					...
					//delete flx[p[1]][p[2]];
					blx = __return_deleted_array(flx); /* flx[p[1]] */
				}
				else {
					blx = flx;
					blx[p[1]][p[2]] = v;
	...
	function add_list_item(e) {
		...
		var tmplData = {
			//title: typeof f[1]!=='undefined'?svx.settings[f[0]].settings.options.title:svx.settings[f[0]].title,
			title: 'Not set',
			type:$(e.target).closest('.svx-option-wrapper').find('.svx-list-selectbox').val(),
			customizer: u(svx.settings[f[0]].supports)!==false&&$.inArray('customizer',svx.settings[f[0]].supports)!==-1?true:false,
		};
	...
	svx.language = u($('#svx-main-wrapper').attr('data-language'))?$('#svx-main-wrapper').attr('data-language').toString().substring(1):null;
	...
	function _ajax_switch_control_option(pnt,f) {
		...
		if ( pnt.hasClass('svx-selectize-active') ) {
			...
			$.each( svx.ajax_factory[f.options], function(i,d) {
				...
				var val = f.val!==false?f.val:f.default;
	...
	function svx_ajax( settings ) {
		...
		if ( u($('#svx-main-wrapper').attr('data-language')) ) {
			data.lang = $('#svx-main-wrapper').attr('data-language').toString().substring(1);
		}
		...
		switch(f.style) {
			...
			case 'text':
				if ( u(f.text) ) {
					return {
						style:f.text.style,
						normal:f.text.normal,
						active:f.text.active,
						disabled:f.text.disabled,
						outofstock:f.text.outofstock,
					};
				}
				else {
					return {
	...
	function term_customizer_add(e) {
		var f =__get_value_by_name($('#svx-customizer').attr('data-id'));
		var l = u(f.options)===false?0:f.options.length;
		if ( u(f.terms) === false ) {
			f.terms = [];
		}
		...
		f.terms.push({id:'c'+l,slug:sanitize_title('Option #'+(l+1)),name:'Option #'+(l+1)});
		f.options.push({id:'c'+l,slug:sanitize_title('Option #'+(l+1)),name:'Option #'+(l+1),value:'',tooltip:'',price:''});
		term_make_items(e,f,Math.floor(Math.max(0, f.terms.length-1)/5)*5);
	...
	function term_make_items(e,f,x) {
		...
		if ( f.taxonomy !== 'meta' && typeof svx.ajax_factory[f.taxonomy] == 'undefined' ) {
	...
	function __term_order(f) {
		if ( u(f.terms) && u(f.options) ) {
			var r = [];
			$.each( f.options, function(i,o) {
				r.push(__get_object_with_propery_slug(f.terms,o.slug));
			} )
			if ( f.terms.length == r.length ) {
				f.terms = r;
			}
			else {
				$.each( f.terms, function(n,p) {
					if ( u(__get_object_with_propery_slug(r,p.slug)) === false ) {
						r.push(p);
						f.options.push({slug:p.slug,name:p.name,id:p.id});
					}
				} );
				f.terms = r;
			}
	...
	function term_put_items(e,f,x) {
		...
		var dO = u(f.taxonomy) == 'meta' || u(f.custom_order) == 'true' ? f.terms : svx.ajax_factory[f.taxonomy];
		var l = dO.length;
		var html = '';
		var template = wp.template( 'svx-customizer-term' );
	...
	function term_fill_settings(e,x) {
		...
		$.each( $('#svx-customizer-terms .svx-terms-list-item'), function(i,p) {
			...
			if ( u(g) ) {
				...
				if ( u(g.value) ) { o.find('input[name="value"]').val(g.value) };
				if ( u(g.price) ) { o.find('input[name="price"]').val(g.price) };
	...
	function set_terms_option(e) {
		...
		if ( v === false ) {
			v = {id:parseInt(item.attr('data-id'),10),name:item.find('input[name="name"]').val(),slug:item.attr('data-slug'),tooltip:item.find('textarea[name="tooltip"]').val(),value:item.find('input[name="value"]').val()};
			svx.settings[s[0]].val[s[1]].options.push(v);
			
		}

		v[$(e.target).attr('name')] = r;

		if ( svx.settings[s[0]].val[s[1]].type == 'ivpa_custom' && $(e.target).attr('name') == 'name' ) {
	...
	function message_alert( message, type, button ) {
		...
		if ( button ) {
			...
			//msgAlertOk.setAttribute('onclick', 'remove_me(this.parentNode.parentNode);');
