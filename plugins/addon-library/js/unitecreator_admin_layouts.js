function UniteCreatorAdmin_LayoutsList(){
	...
	function initEventsCats(){
		...
		*/
		...
		jQuery(document).on('click', '.uc-button-set-category-tolayout', function(){
			setCattegoryForLayout(this);
		});
		
		
		jQuery(".uc-layouts-list-category").click(onChangeCategoryClick);
	...
	*/
	function onChangeCategoryClick(){
		...
		var layoutID = objButton.data("owner");
	...
	*/
	 ...
	 */
	function updateCategoryTitle(categId, newTitle){
		...
		g_ucAdmin.ajaxRequest("update_category", data, function(response){
			...
			jQuery('a.uc-layouts-list-category[data-catid='+categId+']').html(newTitle);
			closeEdit(categId, newTitle);
	...
	*/
	function setCattegoryForLayout(elem){
		
		jQuery(elem).attr('disabled', 'true').html('wait...');
		
		var catID = getSelectedCatIDFromHtmlTable();
		var layout_id = g_openedLayout;
		...
		g_ucAdmin.ajaxRequest("update_layout_category", data, function(response){
			
			jQuery('a.uc-layouts-list-category[data-owner='+layout_id+']').html(getCategoryNameFromHtmlTableById(catID));
			jQuery('a.uc-layouts-list-category[data-owner='+layout_id+']').attr("data-catid", catID).data('catid', catID);
			
			jQuery(elem).removeAttr('disabled').html('Set');
			jQuery('#uc_dialog_add_category').dialog('close');
		});
