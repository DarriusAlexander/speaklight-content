...
defined('ADDON_LIBRARY_INC') or die('Restricted access');
	...
	class UniteFunctionsWPUC_archive{
		...
		public static function getCategoriesHtmlList($catIDs,$strTax = null){
			...
			$arrErrors = UniteFunctionsUC::getVal($categories, "errors");
			...
			if(!empty($arrErrors)){
				...
				UniteFunctionsUC::throwError("getCategoriesHtmlList error: ".$strErrors);
			}
			...
			foreach ( $categories as $category ) {
				...
				if(!empty($link))
					$thelist .= '<a href="' . esc_url( $link ) . '" title="' . esc_attr( sprintf( __( "View all posts in %s", ADDONLIBRARY_TEXTDOMAIN), $category["name"] ) ) . '" ' . $rel . '>' . $catName.'</a>';
				else
					$thelist .= $catName;
		...
		public static function getCatAndTaxData($catIDs){
			...
			foreach($catIDs as $cat){
				...
				$pos = strrpos($cat,"_");
				if($pos === false)
					UniteFunctionsUC::throwError("The category is in wrong format");
		...
		public static function getExcerptById($postID, $limit=55){
			...
			$excerpt = UniteFunctionsUC::getTextIntro($excerpt, $limit);
		...
		public static function getAttachmentImage($thumbID,$size = self::THUMB_FULL){
			...
			$output = array();
			$output["url"] = UniteFunctionsUC::getVal($arrImage, 0);
			$output["width"] = UniteFunctionsUC::getVal($arrImage, 1);
			$output["height"] = UniteFunctionsUC::getVal($arrImage, 2);
		...
		/**
		 * parse attachment post data
		 */
		public static function parseAttachmentPost($post){

			$arrSizes = self::getArrThumbSizes(true);
			...
			$item["description_title"] = !empty($post->post_content)?$post->post_content:$post->post_title;
			
			//put thumb url
			$arrThumb = self::getAttachmentImage($post->ID, self::THUMB_MEDIUM);
			$item["thumb"] = UniteFunctionsUC::getVal($arrThumb, "url");
			$item["thumb_width"] = UniteFunctionsUC::getVal($arrThumb, "width");
			$item["thumb_height"] = UniteFunctionsUC::getVal($arrThumb, "height");
			
			//put all other thumbs url
			foreach($arrSizes as $size => $sizeTitle){
			
				if($size == "medium" || $size == "full")
					continue;
			
				$arrThumb = self::getAttachmentImage($post->ID, $size);
				$item["thumb_{$size}"] = UniteFunctionsUC::getVal($arrThumb, "url");
				$item["thumb_{$size}_width"] = UniteFunctionsUC::getVal($arrThumb, "width");
				$item["thumb_{$size}_height"] = UniteFunctionsUC::getVal($arrThumb, "height");
		...
		/**
		 * get attachment item from post
		 */
		public static function getPostAttachmentImage($postID){
			$item = null;
			$attachmentPostID = self::getPostThumbID($postID);
			if(empty($attachmentPostID))
				return($item);
			
			$post = get_post($attachmentPostID);
			if(empty($post))
				return($item);
			
			$item = self::parseAttachmentPost($post);
		...
		/**
		 * get unitecreator items array from attachment id's
		 */
		public static function getArrItemsFromAttachments($arrIDs){
			...
			$arrItems = UniteFunctionsUC::arrayToAssoc($arrItems, "id");
		 ...
		 */
		public static function getArrThumbSizes($includeSmall = false){
			...
			if($includeSmall == true)
				$arrSizes[self::THUMB_SMALL] = "Small (50x50)";
		...
		public static function deletePost($postID){
			...
			if($success == false)
				UniteFunctionsUC::throwError("Could not delete post: $postID");
		}
		...
		public static function getIntroFromContent($text){
			...
			if(!empty($text)){
				...
				$intro = UniteFunctionsUC::getVal($arrExtended, "main");
		...
		public static function writeSettingLanguageFile($filepath){
			...
			$path = UniteFunctionsUC::getVal($info, "dirname")."/";
			...
			foreach($arrText as $text){
				...
				$str .= "_e(\"$text\",\"".ADDONLIBRARY_TEXTDOMAIN."\"); \n";				
			}
			$str .= "?>";
			
			UniteFunctionsUC::writeFile($str, $filepathOutput);
		}
